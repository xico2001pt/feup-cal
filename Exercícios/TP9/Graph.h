...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    //Vertex<T> * s = findVertex(source); Vertex<T> * f = findVertex(sink);
    //if (s == nullptr || f == nullptr) return INF;

    // 1. Determinar a “distância” mínima de s a todos os vértices
 ...
 */
template <class T>
void Graph<T>::dfs(Vertex<T> * v) const {
    for (Vertex<T> * v : vertexSet)
        v->visited = false;
    dfsVisit(v);
}
